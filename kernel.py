# -*- coding: utf-8 -*-
"""
USTB Chat2API 内核（kernel）：工具调用桥解析器 + 思考区过滤器 + 尖括号预警。

- 纯逻辑模块（除落盘诊断日志外无外部依赖），与外壳（chat2api.py：FastAPI 路由/
  鉴权/生命周期）解耦，支持进程内热重载：importlib.reload(kernel)。
- 外壳通过 `kernel.Xxx` 动态属性引用本模块符号（而非 from-import 旧绑定），
  这样 reload 后新请求立即使用新类；进行中的流式响应用旧实例，完全不受影响。
"""
import json
import os
import re
...
BASE_DIR = os.environ.get("CHAT2API_HOME") or os.path.dirname(os.path.abspath(__file__))
...
def log_warn(msg: str):
    ...
    try:
        with open(os.path.join(BASE_DIR, "server_warn.log"), "a", encoding="utf-8") as f:
