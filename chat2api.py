...
# ===== 内核（kernel.py）：解析器/工具桥/尖括号预警，可进程内热重载 =====
import kernel  # noqa: E402  (同目录内核模块; 外壳经 kernel.X 动态引用以支持热重载)
from kernel import (content_to_text, angle_bracket_check, format_tools_prompt,  # noqa: F401
                    render_tool_calls_xml, extract_tool_calls, parse_invokes_xml,
                    ReasoningXMLFilter, ToolCallStreamParser, TOOL_INSTRUCTION)  # noqa: F401
...
class InFlightMiddleware:
    ...
    流式响应（StreamingResponse）以流被完整消费为结束——保证热更新只在
    「对话间隙」执行，绝不打断进行中的对话；进行中的流式响应绑定的是
    旧 kernel 实例，reload/替换均不影响其正常输出。"""
...
def build_upstream_form(messages: list, include_usage: bool, tools: list = None) -> dict:
    ...
    for m in messages:
        ...
        if role == "developer":
            role = "system"
        content = kernel.content_to_text(m.get("content"))
        if role == "tool":
...
async def chat_completions(request: Request):
    ...
    prompt_chars = sum(len(kernel.content_to_text(m.get("content"))) for m in body.get("messages", []))
    t0 = time.time()
    ...
    if not stream:
        ...
        if calls:
            _log_tools(calls)
        kernel.angle_bracket_check(text, "content", {})      # 尖括号预警（调试阶段）
        kernel.angle_bracket_check(reasoning, "reasoning", {})
    ...
    async def sse():
        ...
        try:
            async for item in stream_upstream_chunks(fields, want_usage):
                if "usage" in item:
                    ...
                    if reasoning:
                        ...
                        if r_txt:
                            kernel.angle_bracket_check(r_txt, "reasoning", warn_r)
                            yield make_chunk(cid, model, {"reasoning_content": r_txt})
                    if text:
                        out_chars += len(text)
                        out_c, out_t = parser.feed(text)
                        if out_c:
                            kernel.angle_bracket_check(out_c, "content", warn_c)
            ...
            if r_rest:
                kernel.angle_bracket_check(r_rest, "reasoning", warn_r)
            ...
            if left:
                out_chars += len(left)
                kernel.angle_bracket_check(left, "content", warn_c)
...
async def restart(request: Request):
    """整进程重启（外壳级更新：路由/入口/依赖变更）。对话间隙感知：
    有活跃对话时挂起，全部结束后自动替换进程。需 API Key 鉴权。"""
    check_auth(request)
    request_restart()
    n = get_inflight()
    if n == 0:
...
async def reload_kernel(request: Request):
    """进程内热重载内核 kernel.py（解析器/工具桥/尖括号预警），不断连接、不换进程。
    对话间隙感知：有活跃对话时挂起，全部结束后自动热重载（毫秒级）。
    适合开发迭代中的快速更新；外壳级变更请用 /restart。需 API Key 鉴权。"""
    check_auth(request)
    n = get_inflight()
    if n > 0:
        with _IDLE_LOCK:
